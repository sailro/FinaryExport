...
- **2026-03-17 — Full project reassessment #3.** Verified all 4 projects build clean (0 warnings), 240/240 tests pass. Found and fixed major architecture.md drift: (1) Project structure still showed single-project layout — rewrote §2 to show Core/CLI/MCP tri-project structure. (2) MCP server completely undocumented — added new §9 covering tools, elicitation auth, AutoInitFinaryApiClient decorator, DI registration, stdio transport. (3) Dependencies table missing ModelContextProtocol 1.1.0. (4) DI section referenced `AddFinaryExport()` (dead name) — updated to `AddFinaryCore()` with split registration pattern. (5) Design Decisions table missing D-mcp and D-mcp-auth. (6) ToC and section numbering fixed. (7) ICredentialPrompt section updated to document both implementations (Console + MCP Elicitation). Decisions.md updated: Architecture Blueprint marked partially superseded (multi-project), D-mcp-complete auth model corrected from "session-only" to "elicitation with warm start fallback", user directive marked partially superseded. Dead config cleaned: removed stale `"Period": "all"` from both appsettings.json files (FinaryOptions has no Period property). MCP code quality: clean — no TODOs, no dead code, no debug artifacts. Package versions all current and consistent.
- **2026-03-17 — Model consolidation analysis (D-model-consolidation).** Reviewed CryptoInfo, FiatInfo, CryptoPosition, FiatPosition, SecurityPosition, SecurityInfo for unification. Findings: CryptoInfo and FiatInfo are byte-for-byte identical (5 properties each, same types) — merge into `AssetInfo`. CryptoPosition and FiatPosition differ only in Id type (`long?` vs `string?`, unused by all consumers) and nested info JSON key (`"crypto"` vs `"fiat"`) — merge into `CurrencyPosition` with dual JSON-mapped properties and computed `Asset` accessor. No custom JSON converters needed. SecurityPosition/SecurityInfo are genuinely different shape (different PnL naming, extra fields like Isin/SecurityType/ExpenseRatio) — leave alone, forced unification would create a God-type. Net impact: 4 types → 2, 3 files → 2, zero JSON behavior change. Half-day implementation for Linus. Decision doc at `.squad/decisions/inbox/rusty-model-consolidation.md`.
