...
// Past dividends detail (filtered by period)
var pastCount = 0;
if (dividends?.PastDividends is { Count: > 0 })
{
var filteredPast = dividends.PastDividends
	.Where(d => PeriodHelper.IsOnOrAfter(d.PaymentAt, cutoff))
	.ToList();
var row = 10;
...
foreach (var div in filteredPast)
{
...
pastCount = filteredPast.Count;
}

// Upcoming dividends detail (not filtered — future-looking data)
var upcomingCount = 0;
if (dividends?.UpcomingDividends is { Count: > 0 })
{
var row = pastCount + 14;
