...
public sealed class UnifiedFinaryApiClientTests
{
	...
	private static readonly FinaryProfile _owner =
		new("org1", "membership-owner", "Jean Dupont");
	private static readonly FinaryProfile _member2 =
		new("org1", "membership-Marie", "Marie Dupont");
	private static readonly FinaryProfile _member3 =
		new("org1", "membership-Claire", "Claire Dupont");

	private static readonly List<FinaryProfile> _allProfiles = [_owner, _member2, _member3];
	...
	{
		var act = () => new UnifiedFinaryApiClient(null!, _allProfiles, _logger);
	...
	{
		var (mock, client) = CreateContextTrackingClient([_owner], out _);
	...
	public async Task GetCategoryAccounts_OwnershipScaling_ComputesFullBalance()
	{
		...
		var (mock, client) = CreateContextTrackingClient([_owner], out var ctx);
		...
		mock.Setup(x => x.GetPortfolioDividendsAsync(It.IsAny<CancellationToken>()))
			.Returns(() => ctx.CurrentMembershipId switch
			{
				"membership-owner" => Task.FromResult<DividendSummary?>(new DividendSummary
				{
					...
					PastIncome = 700m,
					Yield = 3.5m,
					PastDividends = [sharedDiv, ownerDiv],
					UpcomingDividends = []
				}),
				"membership-Marie" => Task.FromResult<DividendSummary?>(new DividendSummary
				{
					...
					PastIncome = 600m,
					Yield = 2.0m,
					PastDividends = [sharedDiv, memberDiv],
					UpcomingDividends = []
				}),
	...
	{
		var mock = new Mock<IFinaryApiClient>();
		var client = new UnifiedFinaryApiClient(mock.Object, _allProfiles, _logger);
	...
	public async Task DelegatedMethods_UseOwnerContext()
	{
		...
		mock.Verify(x => x.SetOrganizationContext(_owner.OrgId, _owner.MembershipId), Times.AtLeastOnce);
	}
